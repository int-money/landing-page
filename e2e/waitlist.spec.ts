...
test.describe("Waitlist Modal", () => {
  ...
  test("should open the waitlist modal when CTA is clicked", async ({ page }) => {
    ...
    const ctaButton = page
      .getByRole("button", { name: /waitlist|join|get started|sign up|get early access/i })
      .first();
  ...
  test("should show validation errors for empty email submission", async ({ page }) => {
    ...
    if (await ctaButton.isVisible().catch(() => false)) {
      ...
      const submitButton = page
        .locator(
          '[role="dialog"] button[type="submit"], [data-state="open"] button[type="submit"], .modal button[type="submit"]'
        )
        .first();
      ...
      if (await submitButton.isVisible().catch(() => false)) {
        ...
        const emailInput = page
          .locator(
            '[role="dialog"] input[type="email"], [data-state="open"] input[type="email"], .modal input[type="email"]'
          )
          .first();
  ...
  test("should accept a valid email in the waitlist form", async ({ page }) => {
    ...
    if (await ctaButton.isVisible().catch(() => false)) {
      ...
      const emailInput = page
        .locator(
          '[role="dialog"] input[type="email"], [data-state="open"] input[type="email"], .modal input[type="email"], input[type="email"]'
        )
        .first();
